...
Definition lemma__lessthanbetween : forall A B C, (euclidean__defs.Lt A B A C) -> ((euclidean__defs.Out A B C) -> (euclidean__axioms.BetS A B C)).
Proof.
intro A.
intro B.
intro C.
intro H.
intro H0.
assert (exists M, (euclidean__axioms.BetS A M C) /\ (euclidean__axioms.Cong A M A B)) as H1 by exact H.
destruct H1 as [M H2].
...
------ apply (@eq__ind__r euclidean__axioms.Point B (fun M0 => (euclidean__axioms.BetS A M0 C) -> ((euclidean__axioms.Cong A M0 A B) -> ((euclidean__axioms.neq A M0) -> ((euclidean__defs.Out A M0 C) -> ((euclidean__defs.Out A C M0) -> (euclidean__axioms.BetS A B C))))))) with (x := M).
-------intro H10.
intro H11.
intro H12.
intro H13.
