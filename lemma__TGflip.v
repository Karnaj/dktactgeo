...
Definition lemma__TGflip : forall A B C a b c, (euclidean__defs.TG A a B b C c) -> ((euclidean__defs.TG a A B b C c) /\ (euclidean__defs.TG A a B b c C)).
Proof.
intro A.
intro B.
intro C.
...
---- assert (* Cut *) (exists h, (euclidean__axioms.BetS a A h) /\ (euclidean__axioms.Cong A h B b)) as H11.
----- apply (@lemma__extension.lemma__extension a A B b H8 H10).
----- destruct H11 as [h H12].
...
---------------- exact H5.
---------------- exact H22.
-------------- assert (euclidean__defs.Lt C c a h) as H24 by exact H19.
split.
--------------- exact H20.
--------------- exact H23.
Qed.
